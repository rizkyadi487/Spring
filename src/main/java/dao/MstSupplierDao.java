...
public interface MstSupplierDao extends JpaRepository<MstSupplier, MstSupplierPK>{
	...
	@Query(value ="select a from MstSupplier a where a.kodeCustomer like :cari or a.namaCustomer like :cari")
	public List<MstSupplier> findDataBySearch(@Param("cari")String cari);
	
	@Query(value ="select a from MstSupplier a where a.kodeCustomer =:cari")
	public MstSupplier findOneku(@Param("cari")String cari);
}