...
public interface MstProvinsiDao extends JpaRepository<MstProvinsi, MstProvinsiPK>{
	...
	@Query(value ="select a from MstProvinsi a where a.kodeCustomer like :cari or a.namaCustomer like :cari")
	public List<MstProvinsi> findDataBySearch(@Param("cari")String cari);
	
	@Query(value ="select a from MstProvinsi a where a.kodeCustomer =:cari")
	public MstProvinsi findOneku(@Param("cari")String cari);
}